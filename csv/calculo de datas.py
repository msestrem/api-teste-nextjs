...
x = int(7) # número de dias a contar da data atual para trás
...
for f in all_files:
    ...
    print(f)
    delta = data_atual - f_date
    print(data_atual)
    print(f_date)
    print("A diferença de dias entre a data do arquivo e a data atual é de: ", delta.days, " dias")
    print()
    ...
    if data_atual <= f_date + timedelta(days=x):
        ...
        print(f, "A data deste arquivo é superior a: ", x, " dias")
        print()
        continue 
        print()
if arquivos_recentes == [ ]:
    print("Não há arquivos a serem considerados neste período da data atual até ", x, "dias atrás")
    print()
    exit()
else:
    print("Estes são os arquivos com data de até ", x, " dias anteriores a data atual: ", data_atual, ": ", arquivos_recentes)
    print()
